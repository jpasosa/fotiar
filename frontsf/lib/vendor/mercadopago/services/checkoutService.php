...
class CheckoutService
{
	public function create_checkout_preference ($checkoutPreferenceData,$accessToken) {//checkoutPreferenceData must be a CheckoutPreferenceData Object, accessToken must be string
      ...
      throw new Exception($preferenceContent);//throw the exception
		...
		// BEGIN DAN
		$cpi_array = array();
		foreach($preferenceContent['items'] as $pc_item):
      $checkoutPreferenceItems = new CheckoutPreferenceItems(); 
      $checkoutPreferenceItems->setItemsId(isset($pc_item['id'])?$pc_item['id']:"");
      $checkoutPreferenceItems->setTitle(isset($pc_item['title'])?$pc_item['title']:"");
      $checkoutPreferenceItems->setDescription(isset($pc_item['description'])?$pc_item['description']:"");
      $checkoutPreferenceItems->setQuantity(isset($pc_item['quantity'])?(int)$pc_item['quantity']:"");
      $checkoutPreferenceItems->setUnitPrice(isset($pc_item['unit_price'])?$pc_item['unit_price']:"");
      $checkoutPreferenceItems->setCurrencyId(isset($pc_item['currency_id'])?$pc_item['currency_id']:"");
      $checkoutPreferenceItems->setPictureUrl(isset($pc_item['picture_url'])?$pc_item['picture_url']:"");
      $cpi_array[] = $checkoutPreferenceItems; 
		endforeach;
    // END DAN
    // BEGIN THEM
//		$checkoutPreferenceItems = new CheckoutPreferenceItems(); 
...
//		$checkoutPreferenceItems->setPictureUrl(isset($preferenceContent['items'][0]['picture_url'])?$preferenceContent['items'][0]['picture_url']:"");
    // END THEM
		...
		// BEGIN DAN
		  foreach($cpi_array as $cpi):
		    $checkoutPreference->setItems($cpi);
		  endforeach;
		// END DAN
    // BEGIN THEM
