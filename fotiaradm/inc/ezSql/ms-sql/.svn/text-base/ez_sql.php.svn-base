<?php
	...
	class db {
		...
		function db($dbuser, $dbpassword, $dbname, $dbhost)
		{
			...
			$this->dbh = odbc_pconnect($dsn, $dbuser, $dbpassword);
		...
		function odbc_rows_affected()
		{
			$result = @odbc_exec($this->dbh, "SELECT @@ROWCOUNT");
			list($affected) = odbc_fetch_row($result);
			return $affected;
		}
		// ==================================================================
		...
		function query($query)
		{
			...
			//$error_res = odbc_exec($this->dbh, $get_errorcode);
			//$errorcode = odbc_result($error_res, "errorcode");
			...
			foreach ( $query_type as $word )
			{
				...
				if ( preg_match("/^$word\s+/i",$query) )
				{
					...
					$this->rows_affected = $this->odbc_rows_affected();
					// This gets the insert ID
					if ( $word == "insert" || $word == "replace" )
					{
						$get_last_ident = "SELECT @@IDENTITY as id";
						$last_res = odbc_exec($this->dbh, $get_last_ident);
						$this->insert_id = odbc_result($last_res, "id");
			...
			else
			{
				...
				while ($i < @odbc_num_fields($this->result))
				{
					...
					$this->col_info[$i]->name = $field->name;
					$this->col_info[$i]->type = $field->type;
					$this->col_info[$i]->size = $field->max_length;
					*/	
					
					$this->col_info[$i]->name = @odbc_field_name($this->result,$i);
					$this->col_info[$i]->type = @odbc_field_type($this->result,$i);
					$this->col_info[$i]->size = @odbc_field_len($this->result,$i);
